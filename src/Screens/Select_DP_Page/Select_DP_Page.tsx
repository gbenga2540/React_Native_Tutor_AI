...
const SelectDPPage: FunctionComponent = () => {
        ...
        const next_dp_action = async ({ action }: { action?: number }) => {
                switch (action) {
                        case 1:
                                navigation.push('AuthStack' as never, { screen: 'VerifyOTPPage' } as never);
                                break;
                        case 2:
                                info_handler({
        ...
        return (
                <View style={styles.select_dp_main}>
                        ...
                        <Text style={styles.su_m_wt}>Select a Display Picture to Proceed</Text>
                        <View style={styles.sdp_w_i_c}>
                                {displayPicture === '' ? (
                                        <Image style={styles.sdp_w_i} source={require('../../Images/Logos/Default_User_Logo.jpg')} />
                                ) : (
                                        <Image
                        ...
                        <BasicButton buttonText={route?.params?.is_change_dp ? 'Upload' : 'Register'} borderRadius={8} marginHorizontal={22} execFunc={upload_data} buttonHeight={56} disabled={disableButton} />
                </View>
        );
};
