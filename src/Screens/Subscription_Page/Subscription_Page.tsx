...
const SubscriptionPage: FunctionComponent = () => {
        ...
        const nav_to_select_payment_page = no_double_clicks({
                execFunc: () => {
                        navigation.push('HomeStack' as never, { screen: 'SelectPaymentPage' } as never);
                },
        });
                        ...
                        <ScrollView style={{ flex: 1 }}>
                                <View
                                        ...
                                        {subscription_data?.map((item, index) => (
                                                <SubscriptionPlan subscription={item} index={item?.id} key={index} activeSubPlan={subPlan} setActiveSubPlan={setSubPlan} />
                                        ))}
                                </View>
                        </ScrollView>
                        <BasicButton buttonText="Continue" marginHorizontal={22} backgroundColor={Colors.Black} textColor={Colors.White} marginTop={'auto'} marginBottom={Platform.OS === 'ios' ? 50 : 20} execFunc={nav_to_select_payment_page} />
                </View>
        );
};
