                ...
                <View style={styles.bap_main}>
                        ...
                        <View style={{ height: 56, marginHorizontal: 22 }}>
                                <HeaderTab
                                        ...
                                        header_2="Blocked"
                                        show_numbers
                                        number_1={allApps?.filter(item => item?.isEnabled === true)?.length}
                                        number_2={allApps?.filter(item => item?.isEnabled === false)?.length}
                                        marginBetween={4}
                        ...
                        {allApps.length > 0 && (
                                <ScrollView
                                        ...
                                        {currentTAB === 1 && allApps.filter(item => item?.isEnabled === true)?.map((item, index) => <AppInfo app={item} key={index} />)}
                                        {currentTAB === 2 && allApps.filter(item => item?.isEnabled === false)?.map((item, index) => <AppInfo app={item} key={index} />)}
                                </ScrollView>
                        )}
                        {allApps.length === 0 && <OverlaySpinner2 showSpinner />}
                        <BasicButton buttonText={currentTAB === 1 ? 'Hide Apps' : 'Show Apps'} marginHorizontal={22} marginTop={'auto'} marginBottom={Platform.OS === 'ios' ? 50 : 20} execFunc={save_blocked_apps} />
                </View>
        );
};
