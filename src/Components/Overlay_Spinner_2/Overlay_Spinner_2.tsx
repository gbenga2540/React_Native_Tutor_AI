...
const OverlaySpinner2: FunctionComponent<ShowSpinnerProps> = ({ showSpinner }) => {
        if (showSpinner) {
                return (
                        <View style={styles.overlay_spinner_main}>
                                <Spinner isVisible={true} size={80} type={spinkit_types[Platform.OS === 'ios' ? 12 : 6] as SpinnerType} color={Colors.Primary} />
                        </View>
                );
        } else {
                return null;
