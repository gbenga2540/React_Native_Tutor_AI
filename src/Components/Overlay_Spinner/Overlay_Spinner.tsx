...
const OverlaySpinner: FunctionComponent<ShowSpinnerProps> = ({ showSpinner, setShowSpinner }) => {
        if (showSpinner) {
                return (
                        <View style={styles.overlay_spinner_main}>
                                <StatusBar barStyle={'light-content'} backgroundColor={Colors.BackgroundDim} />
                                <TouchableOpacity
                                ...
                                <Spinner isVisible={true} size={80} type={spinkit_types[Platform.OS === 'ios' ? 12 : 1] as SpinnerType} color={Colors.Primary} />
                        </View>
                );
        } else {
                return null;
