...
const MicrophoneButton: FunctionComponent<MicrophoneButtonProps> = ({ microphoneSize, marginTop, marginBottom, marginHorizontal, execFunc, disabled, animationSpeed }) => {
        ...
        const micStyle = useAnimatedStyle(() => {
                const color = interpolateColor(micColor.value, [0, 1], [Colors.Primary, Colors.White]);
                return {
                        color: color,
                };
        });
        ...
        return (
                <Pressable
                        ...
                        />
                        <AnimatedFeatherIcon name="mic" size={microphoneSize ? Math.round(microphoneSize / 2.67) : 30} style={[{ position: 'absolute' }, micStyle]} />
                </Pressable>
        );
};
